//excluindo objetos inseridos na aula anterior

//Exercícios sobre Classes - aula 63
...
const removerCarro=(quem)=>{
    a_carros=a_carros.filter((el)=>{   //como não existe o metodo remover, cria-se um filtragem que quando receber o nome do carro que vai ser removido, ele filtra e remove
        return el.nome!=quem //nome diferente de quem
    })
...
const gerenciarExibicaoCarros=()=>{
    ...
    a_carros.forEach((c)=>{
        ...
        const btn=document.createElement("button") //incluindo botão de REMOVER o carro add, cada carro que add vai ter seu proprio botão
        btn.innerHTML="Remover" //dando nome ao botão de excluir
        btn.addEventListener("click",(evt)=>{
            const quemRemover=evt.target.parentNode.dataset.nome//busca o pai da div remover e o datasete dá o nome do carro inderido pelo usuario
            removerCarro(quemRemover)
            gerenciarExibicaoCarros()
            
        })
        div.setAttribute("class","carro")
        div.setAttribute("data-nome",c.nome) // qnd criar um novo carro, pelo data-nome ele reconhece o nome que o usuário inseriu
        ...
        div.appendChild(btn) //formando um botão
        })
    }
